...
- `.claude/settings.json` adds a Claude Code `PreToolUse` hook
  ...
  first runs `ruff check .`, `ruff format --check .`, `python -m mypy` and actionlint,
  and is blocked with the findings if any fails.
...
different interpreter. CI also runs on macOS and Windows: name the encoding on every
text-mode file read or write (`tests/architecture/test_encoding.py` enforces it), build
relative paths for comparison with `.as_posix()`, and mark POSIX-only tests (file modes,
`os.getuid`) with `skipif(sys.platform == "win32")`.
